...
export default {
  ...
  filteredMails: state => state.mails.filter((mail) => {
    // console.log(mail)
    if (state.mail_filter == 'inbox') return mail.mailType == "inbox" && (mail.from.toLowerCase().includes(state.mailSearchQuery.toLowerCase()) || mail.from.toLowerCase().includes(state.mailSearchQuery.toLowerCase()) || mail.subject.toLowerCase().includes(state.mailSearchQuery.toLowerCase()) || mail.message.toLowerCase().includes(state.mailSearchQuery.toLowerCase()))
    if (state.mail_filter == 'sent') return mail.mailType == "sent" && (mail.from.toLowerCase().includes(state.mailSearchQuery.toLowerCase()) || mail.from.toLowerCase().includes(state.mailSearchQuery.toLowerCase()) || mail.subject.toLowerCase().includes(state.mailSearchQuery.toLowerCase()) || mail.message.toLowerCase().includes(state.mailSearchQuery.toLowerCase()))
...
//             */
    ...
    // getMailInbox: state => (mailId) => state.mails.filter((mail) => mail.id_inbox == mailId)[0],
    getMail: state => (mailId) => state.mails.filter((mail) => mail.id_inbox == mailId)[0],
    getMailSent: state => (mailId) => state.mails.filter((mail) => mail.id_sent == mailId)[0],
}
