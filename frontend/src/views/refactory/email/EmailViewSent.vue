                            ...
                            <vx-card class="px-4">
                                ...
                                <div class="vx-row w-full border-b border-l-0 border-r-0 border-t-0 border-grey-light border-solid flex justify-between flex items-center">
                                    <div class="vx-col sm:w-4/5 w-full flex flex-wrap items-center mb-2">
                                        ...
                                        <vs-avatar class="sender__avatar--single flex-no-shrink mr-3 border-2 border-solid border-white" src="http://placehold.it/300" size="65px"></vs-avatar>
                                        <div class="flex flex-col my-2">
                                            <h4 class="mb-1">{{ currentMail.from }}</h4>
                                            <div class="flex items-center">
                                                <span class="text-sm">{{ currentMail.from }}</span>
<!-- 
                                                <vs-dropdown vs-custom-content vs-trigger-click class="inline-flex">
                                                    ...
                                                    <vs-dropdown-menu style="z-index: 40001">
                                                        <div class="p-2">
                                                            <p class="text-sm mb-1">From: <span class="font-semibold"> {{ currentMail.from }} </span></p>
                                                            <p class="text-sm mb-1">To: <span class="font-semibold"> {{ currentMail.to | csv }} </span></p>
                                                            <p class="text-sm mb-1">Date: <span class="font-semibold"> {{ currentMail.created_at | time }} {{ currentMail.time | date(true) }} </span></p>
 ...
 -->
                                    ...
                                    <div class="vx-col sm:w-1/5 w-full flex sm:flex-col items-center sm:justify-end mb-2">
                                            <span class="flex sm:mr-0 mr-2 self-end whitespace-no-wrap">{{ currentMail.created_at | time }}</span>
                                        <span class="flex self-end sm:mt-2 mt-0 whitespace-no-wrap">{{ currentMail.created_at | date(true) }}</span>
                                    </div>
                                </div>
                                    ...
                                    <div class="flex">
                                        <div class="mail__attachment">
                                            ...
                                            <span class="block py-4">
                                                <vs-chip color="primary" class="px-4 py-2 mr-3">{{ currentMail.attachment }}</vs-chip>
                                            </span>
                                        </div>
                                    </div> -->
...
export default{
    ...
    computed: {
        ...
        currentMailLabels: {
            get() {
                return this.$store.getters['surat/getMail'](this.openMailId).labels;
            },
            set(value) {
                if(Array.isArray(value)){
                    const payload = { mailId: this.openMailId, value: value }
                    this.$store.dispatch('surat/updateMailLabels', payload)
                }
            }
        }
    },
