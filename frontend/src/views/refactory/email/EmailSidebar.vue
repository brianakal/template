        ...
        <vs-prompt
            ...
            :vs-active.sync="activePrompt">
                <VuePerfectScrollbar class="scroll-area p-4" :settings="settings">
                    <form @submit.prevent>
                        <vs-input v-validate="'required|email'" name="mailFrom" label-placeholder="From" v-model="mailFrom" class="w-full mb-6" :danger="!validateForm && mailFrom != ''" val-icon-danger="clear" :success="validateForm" val-icon-success="done" :color="validateForm ? 'success' : 'danger'" />
...
export default{
    ...
    data() {
        return {
            activePrompt: false,
            mailFrom: 'Brian@email.com',
            mailTo: 'Whawhan@email.com',
            mailSubject: 'Kirim Email',
            // mailCC: '',
            // mailBCC: '',
            mailMessage: 'Percobaan kirim email',
            editorOption: {
                modules: {
    ...
    computed: {
        validateForm() {
            ...
            return !this.errors.any() && this.mailFrom != '';
        },
        unreadMails() {
            return (mailType) => this.$store.getters['surat/unreadMails'](mailType);
        },
        draftMails() {
            return this.$store.getters['surat/NumdraftMails'];
        },
    },
    mounted() {
        this.registerWS()
    ...
    methods: {
        ...
        async sendEmail() {
            ...
            const response = await axios.post('emails/add_email', data)
            console.log(data)
         },

        registerWS() {
            const portWebsocket = 8008
            const url = 'ws://localhost:'+portWebsocket
            this.connection = new WebSocket(url)

            this.connection.onopen = event => {
                console.log('We are connected at port :'+portWebsocket, event)
            }
